...
namespace Game {
	...
	internal sealed class PlayerCollision : MonoBehaviour {
		...
		private void CheckSideCollision() {
			...
			if (Physics2D.OverlapBox(_playerData.Position + _sideCheckPosition, sideBox, 0.0f, _groundLayer)) {
		...
		private void CheckUpCollision() {
			...
			if (Physics2D.OverlapBox(_playerData.Position + _upCheckPosition, upBox, 0.0f, _groundLayer)) {
		...
		private void OnDrawGizmosSelected() {
			...
			var groundBox = Vector2.right * _groundCheckBoxScale.x + Vector2.up * _groundCheckBoxScale.y;
			Gizmos.DrawWireCube((Vector2)transform.position + _groundCheckPosition, groundBox);

			Gizmos.color = Color.red;
			var sideBox = Vector2.right * _sideCheckBoxScale.x + Vector2.up * _sideCheckBoxScale.y;
			Gizmos.DrawWireCube((Vector2)transform.position + _sideCheckPosition, sideBox);
